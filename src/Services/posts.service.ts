...
class PostsService{
    ...
    async findPost(id: string){
        const res = (await connection.query<Post[]>('SELECT * FROM news WHERE id = ?', [id]))[0][0];
        return res 
    } 
    async createPost(title: string, content: string, imagePath: string){
        const id = v4()
        await connection.query('INSERT INTO news (`id`, `title`, `content`, `img`) VALUES (?, ?, ?, ?)', [id, title, content, imagePath]);
